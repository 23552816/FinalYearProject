...
def load_data(data_dir, userIndex, hyperparam_set):
    """Load the GEFCom 2014 energy load data"""

    energy = pd.read_csv(os.path.join(data_dir, 'user'+str(userIndex)+"_"+str(hyperparam_set)+'.csv'), parse_dates=['timestamp'])
...
def mape(predictions, actuals):
    ...
    absolute_errors = np.absolute(predictions - actuals)
    percentage_errors = np.divide(absolute_errors, actuals, out=np.zeros_like(absolute_errors), where=actuals != 0)
...
class TimeSeriesTensor(UserDict):
    ...
    def __init__(self, dataset, target, H, tensor_structure, freq='30min', drop_incomplete=True):
